...
>这段给出了RPN的结构，如图1左侧。中间的全连接层在空间位置上是共享的，这个还不明白，是窗口之间有重叠所以有共享吗？
...
我们使用单尺度图片[7,5]训练和测试RPN和检测网络。缩放图片使图片的短边长度 s=600 [5]。多尺度特征选择可能提高准确率率但是不能做到速度和准确率的平衡[5]。我们还注意到对于 ZF和VGG 网络，最后一个卷积层在缩放过的图片上总的步幅（stride）是16像素。因此在PASCAL图片（大约500x375）上大约是10像素。即使大的步幅也能得到好的结果，虽然小的步幅可能有更高的准确率。  

对于锚点，我们使用3个尺寸，面积分别是 $128^2,256^2,512^2$，3个宽高比分别是 1：1,1：2 和2：1。我们注意到我们的算法在预测大候选区域时允许使用比感受域大的锚点。这种预测并非不可能——如果只有目标的中间部分可见，仍然可以粗略地推断目标的范围。使用这种设计，我们的方案不需要多尺度特征或者多尺度滑动窗口去预测大区域，同时还要考虑运行时间。图1（右）展示了我们的方法适用于各种尺度和宽高比。下表显示了使用ZF网络的每个锚点学习到的平均候选区域尺寸（s=600）。  
!![](resource/FasterRCNN/table.png)  

超出图像边界的锚点需要细心处理。训练时，我们忽略所有越界（cross-boundary）的锚点。对于1000x600的图片，大约共有20k（约等于60x40x9）个锚点。忽略掉越界（cross-boundary）的锚点，每张图片大约有6k个锚点用于训练。如果训练时步忽略掉越界（cross-boundary）的异常值，它们会在目标函数中引入大的，难以修正的误差项，训练难以收敛。在测试阶段，我们仍将全卷积RPN用于整个图像， 这可能会产生越界候选区域，我们将其剪切到图像边界。  

一些RPN区域彼此高度重叠。为了减少冗余，我们根据其分类(cls)分数对提议区域采用非极大值抑制（NMS）。我们将NMS的IoU阈值固定位0.7，这使得每张图片大约有2k个提议区域。就如我们将要展示的那样，NMS不会损害最终的检测准确率，但是恩能够大量减少提议区域的数量。NMS之后，我们使用排名前N的提议区域用于检测。在下文中，我们使用2k RPN提议区域训练Fast R-CNN，但在测试时评估不同数量的提议。  

>总结：本节讲了
>1.单尺度图片训练（平衡速度和准确率）、  
>2.锚点尺寸（$128^2,256^2,512^2$）和宽高比（1：1,1：2 和2：1）、  
>3.越界锚点的处理（训练时去掉，测试时剪切）、  
>4.重叠区域的处理（NMS筛选）  
